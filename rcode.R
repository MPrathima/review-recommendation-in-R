getwd()
setwd("C:/Users/Krishna Vardhan/Desktop/ta")
getwd()
...
# Reading data
review=read.csv("REVIEW2K.csv", stringsAsFactors = FALSE)
names(review)
...
barplot(review_term_freq[1:20], col = "pink", las = 2)
...
M3=resamples(list(SVM=fit.svm, kNN=fit.knn, glm=fit.glm, RF=fit.rf))
summary(M3)
dotplot(M3)
...
write.csv(pred1,"test.csv")
ts=read.csv("test.csv")
confusionMatrix(pred1,test$recommand)
